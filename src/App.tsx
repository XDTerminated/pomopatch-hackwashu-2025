      ...
      {rainDrops.map((drop) => (
        <div
          ...
          style={{
            ...
            height: "20px",
            background:
              "linear-gradient(to bottom, rgba(174, 194, 224, 0.8), rgba(174, 194, 224, 0))",
...
function App({
  ...
  const [potentialDraggedSeed, setPotentialDraggedSeed] =
    useState<SeedPacket | null>(null);
  ...
  const [weather, setWeather] = useState<WeatherType>(() => {
    ...
    console.log("🌤️ Initializing weather:", {
      initialWeather,
      weatherIndex,
      result: weatherTypes[weatherIndex],
    });
  ...
  const [displayedMoney, setDisplayedMoney] = useState(
    () => initialMoney ?? 100
  );
  ...
  const [pomodoroMode, setPomodoroMode] = useState<"none" | "work" | "break">(
    "none"
  );
  ...
  const [currentVarietyIndices, setCurrentVarietyIndices] = useState<
  ...
  const plantVarieties: Record<
    string,
    { rarity: number; chance: number; varieties: string[] }[]
  > = {
    berry: [
  ...
  useEffect(() => {
    console.log(
      "Rain effect triggered. Current weather:",
      weather,
      "Muted:",
      isMuted
    );
  ...
  useEffect(() => {
    console.log(
      "Birds effect triggered. Current weather:",
      weather,
      "Muted:",
      isMuted
    );
        ...
        const fadeOutInterval = setInterval(() => {
          if (currentAudio && currentAudio.volume > 0.05) {
            ...
            if (currentAudio) {
              ...
              if (weatherAudioRef.current === currentAudio)
                weatherAudioRef.current = null;
            }
            clearInterval(fadeOutInterval);
          }
      ...
      try {
        ...
        setPlacedSprouts((prevSprouts) => {
          return plants.map((plant: Plant) => {
            ...
            const existing = prevSprouts.find(
              (s) => s.id === `plant-${plant.plant_id}`
            );
  ...
  useEffect(() => {
    if (!showLeaderboard) {
      // Reset displayed count when closing
      setDisplayedUsersCount(20);
      return;
    }
    ...
    fetchLeaderboard();
  }, [showLeaderboard, getAuthToken]);

  // Handle infinite scroll for leaderboard
  const handleLeaderboardScroll = () => {
    const scrollElement = leaderboardScrollRef.current;
    ...
    if (
      ...
      displayedUsersCount < leaderboardUsers.length
    ) {
      setDisplayedUsersCount((prev) =>
        Math.min(prev + 20, leaderboardUsers.length)
      );
    }
  };
  ...
  const getPlantSprite = (sprout: PlacedSprout): string => {
    ...
    if (!species) {
      console.warn(
        "🚨 Plant at stage",
        stage,
        "missing species, using basicSprout. Sprout:",
        sprout
      );
      return "/Sprites/basicSprout.png";
    }
  ...
  const isInUIArea = (y: number): boolean => {
  ...
  const handleGlobalMouseUp = () => {
    if ((draggedSeed || draggedTool) && !attachedSproutId) {
      ...
      if (draggedSeed && money >= draggedSeed.price) {
        ...
        const inUIArea = isInUIArea(cursorPosition.y);
        ...
        if (!hasCollision && !inUIArea && !inventoryFull) {
          ...
          getAuthToken()
                ...
                prev.map((s) =>
                  s.id === tempSproutId
                    ? {
                        ...
                        growth_time_remaining:
                          response.growth_time_remaining ??
                          s.growth_time_remaining,
                        fertilizer_remaining:
                          response.fertilizer_remaining ??
                          s.fertilizer_remaining,
                      }
                    : s
                )
              );
            ...
            .catch((error) => {
              ...
              setPlacedSprouts((prev) =>
                prev.filter((s) => s.id !== tempSproutId)
              );
        ...
        if (hoveredSproutId) {
          if (draggedTool.type === "Spade") {
            ...
            const plantIdStr = hoveredSproutId
              .replace("plant-", "")
              .replace("temp-plant-", "");
            const plantId = parseInt(plantIdStr);
            const targetSprout = placedSprouts.find(
              (s) => s.id === hoveredSproutId
            );
            ...
            if (currentStage !== 1) {
              console.log(
                "🌿 Cannot fertilize - plant must be at stage 1 (current stage:",
                currentStage,
                ")"
              );
              playSound("/Audio/error.mp3");
            ...
            if (growthTimeRemaining !== null) {
              console.log(
                "🌿 Cannot fertilize - plant is still growing (time remaining:",
                growthTimeRemaining,
                ")"
              );
              playSound("/Audio/error.mp3");
            ...
            if (
              fertilizerRemaining !== null &&
              fertilizerRemaining !== undefined &&
              fertilizerRemaining <= 0
            ) {
              console.log(
                "🌿 Cannot fertilize - plant doesn't need fertilizer (fertilizer_remaining:",
                fertilizerRemaining,
                ")"
              );
              playSound("/Audio/error.mp3");
            ...
            console.log("🌿 Fertilizing plant:", {
              ...
              currentMoney,
              fertilizerCost,
              newMoney: currentMoney - fertilizerCost,
              fertilizerRemaining,
              growthTimeRemaining,
            });
            ...
            const newFertilizerRemaining = Math.max(
              0,
              ...
              prev.map((s) =>
                s.id === hoveredSproutId
                  ? {
                      ...
                      growth_time_remaining: shouldStartGrowing
                        ? s.rarity === 0
                          ? 60
                          : s.rarity === 1
                          ? 120
            ...
            if (currentStage >= 1) {
              console.log(
                "💧 Cannot water - plant already at stage",
                currentStage
              );
              playSound("/Audio/error.mp3");
              setHoveredSproutId(null);
            ...
            if (growthTimeRemaining !== null) {
              console.log(
                "💧 Cannot water - plant is still growing (time remaining:",
                growthTimeRemaining,
                ")"
              );
              playSound("/Audio/error.mp3");
            ...
            console.log("💧 Watering plant:", {
              ...
              currentMoney,
              waterCost,
              newMoney: currentMoney - waterCost,
            });
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (selectedSeed && !attachedSproutId) {
      ...
      if (money >= selectedSeed.price) {
        ...
        const inUIArea = isInUIArea(e.clientY);
        const inventoryFull = placedSprouts.length >= inventoryLimit;
          ...
          if (!isNaN(plantId) && !sproutId.startsWith("temp-")) {
            if (money >= 25) {
              ...
              if (currentStage === 0 && sprout.growth_time_remaining === null) {
                ...
                setPlacedSprouts((prev) =>
                  prev.map((s) =>
                    s.id === sproutId ? { ...s, growth_time_remaining: 30 } : s
                  )
                );
                setSelectedTool(null); // Deselect after using
                ...
                for (let i = 0; i < particleCount; i++) {
                  const angle =
                    (Math.PI * 2 * i) / particleCount + Math.random() * 0.3;
          ...
          if (!isNaN(plantId) && !sproutId.startsWith("temp-")) {
            if (money >= 25) {
              ...
              if (
                currentStage === 1 &&
                fertilizerNeeded &&
                sprout.growth_time_remaining === null
              ) {
                // Update client-side first (capture previous money for rollback)
                const prevMoney = money;
                setMoney(prevMoney - 25);
                const newFertilizerRemaining = Math.max(
                  0,
                  (sprout.fertilizer_remaining ?? 0) - 1
                );
                const shouldStartGrowing = newFertilizerRemaining === 0;
                setPlacedSprouts((prev) =>
                  prev.map((s) =>
                ...
                getAuthToken()
                  .then((token) => {
                    if (!token) throw new Error("Failed to get auth token");
                    return apiService.applyFertilizer(
                      userEmail,
                      plantId,
                      token
                    );
    ...
    if (attachedSproutId) {
      ...
      if (isHoveringDollarSign) {
        ...
        if (sproutToSell) {
          const plantIdStr = attachedSproutId
            .replace("plant-", "")
            .replace("temp-plant-", "");
          const plantId = parseInt(plantIdStr);
      ...
      if (!hasCollision && !inUIArea) {
        ...
        console.log("🚚 Moving plant:", {
          plantId,
          isTemp: attachedSproutId.startsWith("temp-"),
          relativeX,
          relativeY,
        });
        setPlacedSprouts(
  ...
  const handleClaimPomodoroReward = async () => {
    ...
    setPlacedSprouts((prev) =>
      prev.map((sprout) => {
        ...
        if (currentGrowth === null || currentGrowth === undefined)
          return sprout;
    ...
    <>
      ...
      >
        ...
        {(isViewingOtherGarden ? viewedPlants : placedSprouts).map((sprout) => {
          const isAttached =
            !isViewingOtherGarden && attachedSproutId === sprout.id;
          const isHovered =
            hoveredSproutId === sprout.id &&
            pomodoroMode !== "work" &&
            !showLeaderboard &&
            !showSearchModal;
          const isNewlyPlaced =
            !isViewingOtherGarden && newlyPlacedSproutId === sprout.id;
          const hasCollision =
            isAttached &&
          ...
          if (
            stage === 1 &&
            growthTime === null &&
            (fertilizerNeeded === null || fertilizerNeeded === undefined)
          ) {
          ...
          return (
            ...
            >
                ...
                style={{
                  ...
                  filter:
                    isAttached && isHoveringDollarSign
                      ? "sepia(100%) saturate(500%) hue-rotate(60deg) brightness(1.2)"
                      : hasCollision || inUIArea
                      ? "sepia(100%) saturate(500%) hue-rotate(-50deg) brightness(0.8)"
                      : isHovered
                      ? "brightness(1.3) saturate(0.9)"
                      : "none",
              ...
              {!isAttached && stage === 0 && growthTime === null && (
                <div
                  className="absolute flex items-center gap-1 wiggle"
                  style={{ zIndex: 100, top: "8px", right: "8px" }}
                >
                  <img
                    src="/Sprites/UI/wateringcan.png"
              ...
              {!isAttached &&
                stage === 1 &&
                growthTime === null &&
                fertilizerNeeded !== null &&
                fertilizerNeeded !== undefined &&
                fertilizerNeeded > 0 && (
                  <div
                    className="absolute flex items-center gap-1 wiggle"
                    style={{ zIndex: 100, top: "8px", right: "8px" }}
              ...
              {!isAttached &&
                ...
                growthTime > 0 && (
                  <div
                    ...
                    style={{ zIndex: 100, top: "calc(100% - 30px)" }}
                  >
                    <span
        ...
        {hoveredSproutId &&
          ...
          (() => {
            const hoveredSprout = (
              isViewingOtherGarden ? viewedPlants : placedSprouts
            ).find((s) => s.id === hoveredSproutId);
              ...
              >
                <div className="flex flex-col gap-1">
                  ...
                  {stage === 0 ? (
                    ...
                    <div
                      className={`font-bold ${
                        hoveredSprout.rarity === 0
                          ? "text-blue-800"
                          : hoveredSprout.rarity === 1
                          ? "text-purple-800"
                          : "text-amber-800"
                      }`}
                    >
                      {(hoveredSprout.species || "Unknown")
                        .split("_")
                        .map(
                        ...
                        .join(" ")}{" "}
                      (
                      {(hoveredSprout.seedType || "").charAt(0).toUpperCase() +
                        (hoveredSprout.seedType || "").slice(1)}
                      )
                    </div>
                  )}
                  ...
                  {stage === 0 && growthTime === null && (
                    <div className="text-blue-800">💧 Water needed</div>
                  )}
                </div>
              </div>
            );
        ...
        {isDraggingOverBackground &&
          ...
          (() => {
            ...
            const inUIArea = isInUIArea(dragPosition.y);
        ...
        <div className="h-full w-full flex flex-col justify-between relative z-0">
          <div className="flex flex-row justify-between h-fit w-full p-8">
            <div className="flex flex-row gap-4 items-start">
                      ...
                      {seedPackets.map((seed) => {
                        ...
                        return (
                          <li
                            ...
                            onMouseDown={
                              canAfford
                                ? handleSeedMouseDown(seed)
                                : () => playSound("/Audio/error.mp3")
                            }
                            onClick={
                              canAfford ? handleSeedClick(seed) : undefined
                            }
                            onMouseEnter={() => {
                              playSound("/Audio/interact.mp3");
                            ...
                            className={`size-16 flex justify-center items-center flex-col gap-0.5 transition-all ${
                              ...
                              selectedSeed?.id === seed.id
                                ...
                                : canAfford
                                ? "opacity-100 cursor-grab active:cursor-grabbing active:scale-95"
                                : "opacity-30 cursor-not-allowed"
                            } ${
                              !draggedSeed && canAfford ? "wiggle-hover" : ""
                            }`}
                            style={{
                          ...
                          >
                            ...
                            {isHovered && plantVarieties[seed.id] && (
                              <div className="absolute inset-0 flex justify-center items-center pointer-events-none">
                                {plantVarieties[seed.id].map(
                                  (rarityGroup, rarityIndex) => {
                                    // Determine position based on number of rarity groups
                                    const totalGroups =
                                      plantVarieties[seed.id].length;
                                    let leftPosition = "50%";
                                    ...
                                    const varietyIndex =
                                      currentVarietyIndices[varietyKey] || 0;
                                    const currentVariety =
                                      rarityGroup.varieties[varietyIndex];
                                    ...
                                    return (
                                      ...
                                      >
                                        <img
                                          src={`/Sprites/${plantType}/${currentVariety}_2.png`}
                            ...
                            <div
                              className={`text-sm pointer-events-none font-bold ${
                                !canAfford ? "text-red-600" : ""
                              }`}
                              style={{ color: !canAfford ? "" : "#9e4539" }}
                            >
                              ${seed.price}
                            </div>
                          </li>
                            ...
                            <li
                              ...
                              onClick={async (e) => {
                                if (tool.type === "Backpack" && canAffordTool) {
                                  try {
                                    ...
                                    // Then sync with backend
                                    const result =
                                      await apiService.increasePlantLimit(
                                        userEmail || "",
                                        token
                                      );
                                    console.log(
                                      "Plant limit upgrade result:",
                                      result
                                    );
                                    ...
                                    setInventoryLimit(
                                      result.new_plant_limit ??
                                        result.plant_limit ??
                                        inventoryLimit + 25
                                    );
                                  } catch (error) {
                                    console.error(
                                      "Failed to upgrade plant limit:",
                                      error
                                    );
                                    // Rollback on error
                                    setMoney(money);
                              ...
                              className={`size-16 flex justify-center items-center flex-col gap-0.5 transition-all relative ${
                                tool.type === "Backpack"
                                  ...
                                  ? "opacity-50 cursor-grab active:cursor-grabbing active:scale-95"
                                  : attachedSproutId || !canAffordTool
                                  ? "opacity-30 cursor-not-allowed"
                                  : "opacity-100 cursor-grab active:cursor-grabbing active:scale-95"
                              } ${
                                !draggedTool &&
                              ...
                              style={{
                                pointerEvents: attachedSproutId
                                  ? "none"
                                  : "auto",
                                filter: !canAffordTool
                                  ? "grayscale(100%)"
                                  : "none",
                                flexShrink: 0,
                              }}
                            >
                              ...
                              {hoveredToolId === tool.id &&
                                (tool.type !== "Backpack"
                                  ...
                                  >
                                    {tool.type === "WateringCan" &&
                                      "Enables sprouts to grow"}
                                    {tool.type === "Fertilizer" &&
                                      "Enables growth to maturity"}
                                    {tool.type === "Backpack" &&
                                      `Extra Plant Slots (+25)`}
                                  </div>
                                )}
                            </li>
                          );
              ...
              <div
                className="flex flex-col items-center relative self-center ml-8 -mt-6"
            ...
            <div className="flex flex-col items-end gap-2">
              <div className="flex flex-row items-center gap-3">
                ...
                >
                  {(() => {
                    const displayName = isViewingOtherGarden
                      ? viewedUser?.username ||
                    ...
                    return hashIndex !== -1
                      ? displayName.substring(0, hashIndex)
                      : displayName;
                  })()}
              ...
              <div
                className="text-4xl text-white font-bold relative"
                style={{
                  filter: "drop-shadow(2px 2px 4px rgba(0, 0, 0, 0.5))",
                  zIndex: -1,
                }}
              >
                $
                {Math.round(
                  isViewingOtherGarden ? viewedUser?.money ?? 0 : displayedMoney
                ).toLocaleString()}
              </div>
              <div className="flex flex-row items-center gap-3">
                <button
                ...
                >
                  <img
                    src={
                      isMuted
                        ? "/Sprites/UI/soundOFF.png"
                        : "/Sprites/UI/soundON.png"
                    }
                ...
                >
                  <img
                    src="/Sprites/UI/leaderboard.png"
                    alt="Leaderboard"
                    className="image-pixelated w-9 h-auto"
                    style={{
                      filter: "drop-shadow(2px 2px 4px rgba(0, 0, 0, 0.5))",
          ...
          <div className="p-8 flex flex-row justify-between items-end">
            <div className="flex flex-row gap-4 items-end">
                ...
                tools
                  ...
                  .map((tool) => {
                    const showDollarSign =
                      tool.type === "Spade" && attachedSproutId;
              ...
              >
                {(isViewingOtherGarden
                  ? viewedPlants.length
                  : placedSprouts.length
                ).toLocaleString()}
                /
                {(isViewingOtherGarden
                  ? viewedUser?.plant_limit ?? 0
                  : inventoryLimit
                ).toLocaleString()}
                ...
                <>
                  ...
                  {pomodoroMode === "break" && (
                    <div className="flex flex-col items-center gap-2">
                      ...
                      >
                        Break #{currentBreakNumber}{" "}
                        {currentBreakNumber % 4 === 0 ? "(Long)" : "(Short)"}
                      </div>
                      <div
                      ...
                      {breakTime === 0 ? (
                        <button
                          ...
                          className={`px-8 py-4 text-2xl font-bold text-white transition-all active:scale-95 ${
                            isClaimingReward
        ...
        {pomodoroMode === "work" && (
          <>
            ...
            >
              ...
              >
                Pomodoro #{currentPomodoroNumber}
              </div>
              <div
              ...
              {pomodoroCompleted ? (
                <button
                  ...
                  onMouseEnter={() => playSound("/Audio/interact.mp3")}
                  disabled={isClaimingReward}
                  className={`px-8 py-4 text-3xl font-bold text-white transition-all active:scale-95 ${
                    isClaimingReward
                      ? "opacity-50 cursor-not-allowed"
                      : "wiggle-hover"
                  }`}
        ...
        {showLeaderboard && (
          <>
            ...
            >
              ...
              >
                <style>{`
                                    .w-full.overflow-y-auto::-webkit-scrollbar {
                                        display: none;
                                    }
                                `}</style>
                {isLoadingLeaderboard ? (
                  <div className="text-white text-center text-2xl py-8">
                    Loading...
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                                ...
                                if (hashIndex !== -1) {
                                  return (
                                    <>
                                      ...
                                      >
                                        {username.substring(0, hashIndex)}
                                      </span>
                                      <span style={{ color: "#cd683d" }}>
                                        {username.substring(hashIndex)}
                                      </span>
                                    </>
                                  );
                                }
                    ...
                    {/* Loading indicator when there are more users */}
                    {displayedUsersCount < leaderboardUsers.length && (
                      <div
                        className="text-white text-center text-lg py-4"
                        style={{
            ...
            >
              ...
              <div className="w-full flex flex-col gap-4">
                    ...
                    onKeyDown={async (e) => {
                      if (
                        ...
                        try {
                          const token = await getAuthToken();
                          if (token) {
                            const user = await apiService.getUserByUsernameTag(
                              searchUsername.trim(),
                              searchTag.trim(),
                              token
                            );
                ...
                <button
                  ...
                  onMouseEnter={() => playSound("/Audio/interact.mp3")}
                  disabled={
                    isSearching || !searchUsername.trim() || !searchTag.trim()
                  }
                  className={`px-8 py-4 text-2xl font-bold text-white transition-all ${
                    isSearching || !searchUsername.trim() || !searchTag.trim()
                      ? "opacity-50 cursor-not-allowed"
                      : searchNotFound
                      ? "active:scale-95 wiggle"
                      : "active:scale-95 wiggle-hover"
                  }`}
                  style={{
                    backgroundColor: searchNotFound ? "#F44336" : "#4CAF50",
                    border: searchNotFound
                      ? "3px solid #C62828"
                      : "3px solid #2E7D32",
                    boxShadow: "0 2px 4px rgba(0, 0, 0, 0.5)",
                    imageRendering: "pixelated",
